...
public class FPSPlayerMove : MonoBehaviour {
	...
	void Move(float h,float v){
        ...
        //若玩家垂直或水平轴有输入，则表示玩家进行位移，设置动画参数，将isMove布尔型参数设置为true，播放玩家奔跑动画
        if (h != 0.0f || v != 0.0f) animator.SetBool("isMove", true);
        //若玩家垂直或水平轴无输入，则表示玩家没有位移，设置动画参数，将isMove布尔型参数设置为false，停止播放玩家奔跑动画
